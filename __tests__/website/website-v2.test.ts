...
describe('Marketing Site Enhanced - AIKIT-110', () => {
  ...
  describe('Enhancement: Performance Optimizations', () => {
    ...
    it('should use modern image formats', () => {
      expect(htmlContent).toMatch(/\.webp|\.avif|srcset/i);
    });
  ...
  describe('Enhanced Accessibility', () => {
    ...
    it('should have proper color contrast (WCAG AA)', () => {
      // Check for light text on dark background
  ...
  describe('Enhanced Mobile Experience', () => {
    it('should have mobile-specific optimizations', () => {
      expect(htmlContent).toContain('@media (max-width');
    });

    it('should have touch-friendly button sizes', () => {
      expect(htmlContent).toMatch(/min-height.*44px|min-width.*44px/);
    });
    ...
    it('should disable zoom on inputs (UX best practice)', () => {
      expect(htmlContent).toMatch(/maximum-scale=1|user-scalable=no/);
    });

    it('should have optimized font sizes for mobile', () => {
      expect(htmlContent).toContain('clamp(');
    });
  });

  describe('Enhanced Code Examples', () => {
    it('should have copy button for all code blocks', () => {
      expect(htmlContent).toContain('copy-code');
    });

    it('should have line numbers in code examples', () => {
      expect(htmlContent).toContain('line-number');
    });

    it('should highlight specific code lines', () => {
      expect(htmlContent).toContain('highlight-line');
    });

    it('should have code language indicators', () => {
      expect(htmlContent).toContain('language-');
    });
  });
  ...
  describe('Performance Budget', () => {
    ...
    it('should have minimal inline JavaScript', () => {
      const scriptMatches = htmlContent.match(/<script>[\s\S]*?<\/script>/g) || [];
      const totalScriptSize = scriptMatches.join('').length;
      // Should be less than 10KB of inline JS
      expect(totalScriptSize).toBeLessThan(10240);
    });

    it('should have efficient CSS selectors', () => {
      // No universal selectors in critical styles
      const criticalCSS = htmlContent.match(/<style>[\s\S]*?<\/style>/)?.[0] || '';
      expect(criticalCSS).not.toMatch(/\* \{.*position: absolute/);
    });
  });
